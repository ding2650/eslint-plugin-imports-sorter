...
function isImportError(currentNode, previousNode, ctx) {
    ...
    const currentCommits = currentNode.leadingComments
    const commintCount = currentCommits?.length || 0
    // /* 权重不等，校验中间是否含有换行 */
    if (itemWeight !== preWeight && startLine === endLine - 1 - commintCount) {
...
function getTargetNodeByWeight(node, allNodes) {
    const { zIndex: currentWeight, deepth: currentDeep } = setNodeWeight(node)
    return allNodes.find(item => {
        const { zIndex: itemWeight, deepth: itemDeep } = setNodeWeight(item)
        ...
        return {
            'ImportDeclaration'(node) {
                ...
                if (isError) {
                    ...
                    const [_, isCheckDeepth] = ctx.options
                    const targetNode = isCheckDeepth ? getTargetNode(node, allImportNodes) : getTargetNodeByWeight(node, allImportNodes)
                    const startToken = sourceCode.getTokenBefore(node)
                    const endToken = sourceCode.getLastToken(node)
                    const range = [startToken.range[1], endToken.range[1]]
                    ctx.report({
