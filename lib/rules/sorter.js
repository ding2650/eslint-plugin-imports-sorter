...
function isImportError(currentNode, previousNode, ctx) {
    ...
    const itemStart = currentNode.start
    const preEnd = previousNode.end
    ...
    /* 权重不等，校验中间是否含有换行 */
    if (itemWeight !== preWeight && preEnd === itemStart - 1) {
        return {
            isError: true,
    ...
    create: function (ctx) {
        ...
        return {
            'ImportDeclaration'(node) {
                ...
                const isReactOrderError = isReact && node.source.value.startsWith('react') && !isFirst
